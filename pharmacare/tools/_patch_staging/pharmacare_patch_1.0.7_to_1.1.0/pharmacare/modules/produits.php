...
if ($action === 'import_template') {
    ...
    fputcsv($out, ['nom', 'reference', 'categorie', 'fournisseur', 'stock', 'seuil_alerte', 'prix_achat', 'prix_vente', 'tva', 'date_expiration', 'description'], ';');
    fputcsv($out, ['Paracétamol 500mg', 'MED-001', 'Antalgiques', 'PharmaPlus', '100', '10', '150', '200', '9', '31/12/2027', 'Boîte de 16 comprimés'], ';');
    fputcsv($out, ['Amoxicilline 1g', 'MED-002', 'Antibiotiques', '', '50', '10', '320', '450', '9', '', ''], ';');
    fclose($out);
    exit;
}
    ...
    if (!$file || ($file['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK || !is_uploaded_file($file['tmp_name'] ?? '')) {
        ...
        if (!$fh) {
            ...
            if ($canProcess) {
                ...
                $stmtIns   = $db->prepare("INSERT INTO produits (nom,reference,categorie_id,fournisseur_id,description,stock,stock_magasin,seuil_alerte,prix_achat,prix_vente,tva,date_expiration) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
                $stmtUpd   = $db->prepare("UPDATE produits SET nom=?,categorie_id=?,fournisseur_id=?,description=?,seuil_alerte=?,prix_achat=?,prix_vente=?,tva=?,date_expiration=?,stock_magasin=stock_magasin+? WHERE id=?");
                ...
                while (($row = fgetcsv($fh, 0, $delim)) !== false) {
                    ...
                    if ($existingId !== null) {
                        ...
                        $stmtUpd->execute([$nom, $catId, $fournId, $desc, $seuil, $pa, $pv, $tva, $exp, $stock, $existingId]);
                        if ($stock > 0) $stmtMvtMag->execute([$existingId, $stock, 'Import CSV — ravitaillement magasin', $uid]);
                        $maj++;
                    } else {
                        $stmtIns->execute([$nom, $reference, $catId, $fournId, $desc, 0, $stock, $seuil, $pa, $pv, $tva, $exp]);
                        if ($stock > 0) {
    ...
    <div class="card" style="max-width:820px;margin:0 auto;">
      ...
      <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-grid">
          ...
          <div class="form-group full">
            <small>
              Colonnes reconnues : <strong>nom</strong>* (requis), reference, categorie, fournisseur,
