    ...
    if (!$file || ($file['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK || !is_uploaded_file($file['tmp_name'] ?? '')) {
        ...
        if (!$fh) {
            ...
            if ($canProcess) {
                ...
                $stmtIns   = $db->prepare("INSERT INTO produits (nom,reference,categorie_id,fournisseur_id,description,stock,seuil_alerte,prix_achat,prix_vente,tva,date_expiration) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
                $stmtUpd   = $db->prepare("UPDATE produits SET nom=?,categorie_id=?,fournisseur_id=?,description=?,stock=?,seuil_alerte=?,prix_achat=?,prix_vente=?,tva=?,date_expiration=? WHERE id=?");
                $ligne = 1;
                while (($row = fgetcsv($fh, 0, $delim)) !== false) {
                    ...
                    if ($existingId !== null) {
                        // mode update
                        $stmtUpd->execute([$nom, $catId, $fournId, $desc, $stock, $seuil, $pa, $pv, $tva, $exp, $existingId]);
                        $maj++;
                    } else {
                        $stmtIns->execute([$nom, $reference, $catId, $fournId, $desc, $stock, $seuil, $pa, $pv, $tva, $exp]);
                        $ajoutes++;
                    }
                }
            }
