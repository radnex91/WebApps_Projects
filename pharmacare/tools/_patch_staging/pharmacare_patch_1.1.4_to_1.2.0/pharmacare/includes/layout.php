...
<div id="sidebar">
  ...
  <nav>
    ...
    $showRapports  = $mVentesHist || $mRapports || $mRapportsCaissier || $mCompta;
    $showAdmin     = $mUtilisateurs || $mRemiseAppr || $mRoles || $mCategories || $mPharmacies || $mParametres || $mMenus || $mLicence || $mSauvegarde;
    ?>
...
?>
<script>window.CSRF_TOKEN = <?= json_encode(csrf()) ?>; window.APP_URL = <?= json_encode(APP_URL) ?>; window.PHARMCARE_PAGE = <?= json_encode($GLOBALS['__pc_active_page'] ?? '') ?>;</script>
