...
{
    return [
        'search_product' => ['chercher', 'rechercher', 'trouver', 'ou est', 'ou est', 'lors de', 'paracetamol', 'produit', 'medicament', 'molecule', 'reference', 'code', 'avoir', 'disponibilite', 'disponible', 'prix de', 'combien coute', 'coute combien'],
...
function tool_search_products(string $q): array
{
    ...
    [$pid] = assistant_user_pharmacie();

    $db = getDB();
    $esc = str_replace(['\\', '%', '_'], ['\\\\', '\%', '\_'], $term);
    $sql = "SELECT p.id, p.nom, p.reference, p.prix_vente, COALESCE(pp.stock,0) AS stock
            FROM produits p
    ...
    if (!$rows) {
        return ['reply' => "Aucun médicament trouvé pour « " . $term . " » (pharmacie courante).",
            'quick' => ['Chercher un autre médicament'],
            'links' => $catalogLink ? [$catalogLink] : []];
    }
    $lines = [];
    foreach ($rows as $r) {
        // Quantité visible seulement si droit stock ; sinon disponibilité binaire (pas de fuite)
        $stockTxt = (int)$r['stock'] > 0
            ? ($canStock ? ((int)$r['stock'] . ' en stock') : 'disponible')
            : 'rupture';
        $lines[] = "• " . $r['nom'] . " — réf. " . $r['reference']
            . " — " . fmtMoney((float)$r['prix_vente']) . " (" . $stockTxt . ")";
    }
    $reply = "J'ai trouvé " . count($rows) . " médicament(s) pour « " . $term . " » :\n" . implode("\n", $lines);
