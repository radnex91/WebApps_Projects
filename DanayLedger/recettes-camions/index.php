...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    $action = $_POST['action'] ?? '';
    rateLimit('recettes_camions_' . ($action ?: 'crud'));
    ...
    if ($action === 'create' && hasPermission('recettes_camions_create')) {
        ...
        try {
            ...
            header('Content-Type: application/json');
            echo json_encode(['success'=>true,'message'=>'Recette camion crÃ©Ã©e avec succÃ¨s.','csrf_token'=>generateCSRFToken()]);
        } catch (Exception $e) {
            header('Content-Type: application/json');
    ...
    if ($action === 'update' && hasPermission('recettes_camions_edit')) {
        ...
        if (!$recette || $recette['statut'] !== 'en_attente') {
            header('Content-Type: application/json');
            echo json_encode(['success'=>false,'message'=>'Recette camion introuvable ou dÃ©jÃ  traitÃ©e.']);
            exit;
        }
        // VÃ©rifier propriÃ©tÃ© (non-admin ne peut modifier que ses propres enregistrements)
        if (!hasPermission('admin') && (int)$recette['created_by'] !== (int)$_SESSION['user_id']) {
            header('Content-Type: application/json');
            echo json_encode(['success'=>false,'message'=>'Vous n\'Ãªtes pas autorisÃ© Ã  modifier cet enregistrement.']);
            exit;
        }
        ...
        try {
            ...
            header('Content-Type: application/json');
            echo json_encode(['success'=>true,'message'=>'Recette camion modifiÃ©e.']);
        } catch (Exception $e) {
            header('Content-Type: application/json');
            echo json_encode(['success'=>false,'message'=>'Erreur : ' . $e->getMessage()]);
        }
...
// Actions POST (validate / cancel only â€” create/update handled by AJAX endpoint above)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verifyCSRFToken($_POST['csrf_token'] ?? '')) {
    ...
    if ($action === 'validate' && hasPermission('recettes_camions_validate')) {
        ...
        addNotification(null, 'success', 'Recette camion validÃ©e', 'La recette camion #' . $id . ' a Ã©tÃ© validÃ©e.');
        setFlash('success', 'Recette camion validÃ©e avec succÃ¨s.');
        ...
        setFlash('warning', 'Recette camion annulÃ©e.');
    } elseif ($action === 'delete' && hasPermission('recettes_camions_delete')) {
        $id = (int)($_POST['id'] ?? 0);
        $stmt = $db->prepare("SELECT * FROM recettes_camions WHERE id = ?");
        $stmt->execute([$id]);
        ...
        if ($oldRecord) {
            ...
            $db->prepare("DELETE FROM recettes_camions WHERE id = ?")->execute([$id]);
            addAuditLog('delete', 'recette_camion', $id, [], $oldRecord);
            setFlash('success', 'Recette camion supprimÃ©e.');
        } else {
            setFlash('error', 'Recette camion introuvable.');
        }
    }
...
<div class="main-content" id="main-content" role="main">
    <header class="main-header" role="banner">
        ...
        <div class="header-right">
            <div class="dropdown">
                <button class="notif-btn" aria-label="Notifications" data-bs-toggle="dropdown"><i class="bi bi-bell"></i></button>
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-end">
                    ...
                    <a class="dropdown-item text-danger" href="<?php echo APP_URL; ?>/logout.php"><i class="bi bi-box-arrow-right me-2"></i>DÃ©connexion</a>
                </div>
            </div>
        </div>
    </header>
    ...
    <div class="page-content fade-in">
        ...
        <div class="card mb-3"><div class="card-body">
            <form method="GET" class="row g-2 align-items-end">
                ...
                <div class="col-md-2"><label for="filter_vehicule_id" class="form-label">Camion</label><select name="vehicule_id" id="filter_vehicule_id" class="form-select"><option value="">Tous</option><?php foreach($vehicules as $v): ?><option value="<?php echo $v['id']; ?>" <?php echo $filterVehicule == $v['id'] ? 'selected' : ''; ?>><?php echo e($v['immatriculation']); ?></option><?php endforeach; ?></select></div>
        ...
        <div class="card"><div class="table-container">
            <table class="table" aria-label="Liste des recettes camions">
                <thead><tr>
                    <th>RÃ©f</th><th>Date</th><th>CatÃ©gorie</th><th>Camion</th><th>Agence dÃ©part</th><th>Agence arrivÃ©e</th><th>Client</th><th>Montant</th><th>Statut</th><th>Actions</th>
                </tr></thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-btns">
                                ...
                                <form method="POST" style="display:inline"><input type="hidden" name="csrf_token" value="<?php echo generateCSRFToken(); ?>"><input type="hidden" name="id" value="<?php echo $r['id']; ?>"><input type="hidden" name="action" value="validate"><button type="submit" class="btn btn-sm btn-outline-success" aria-label="Valider" data-confirm="Valider cette recette ?" title="Valider"><i class="bi bi-check-lg"></i></button></form>
                                <form method="POST" style="display:inline"><input type="hidden" name="csrf_token" value="<?php echo generateCSRFToken(); ?>"><input type="hidden" name="id" value="<?php echo $r['id']; ?>"><input type="hidden" name="action" value="cancel"><button type="submit" class="btn btn-sm btn-outline-danger" aria-label="Supprimer" data-confirm="Annuler cette recette ?" title="Annuler"><i class="bi bi-x-lg"></i></button></form>
                                <?php endif; ?>
                                <?php if (hasPermission('recettes_camions_delete')): ?>
                                <form method="POST" style="display:inline"><input type="hidden" name="csrf_token" value="<?php echo generateCSRFToken(); ?>"><input type="hidden" name="id" value="<?php echo $r['id']; ?>"><input type="hidden" name="action" value="delete"><button type="submit" class="btn btn-sm btn-outline-danger" aria-label="Supprimer" data-confirm="Supprimer cette recette camion ?" title="Supprimer"><i class="bi bi-trash"></i></button></form>
                                <?php endif; ?>
                            </div>
                        </td>
                    </tr>
                ...
                <?php if (empty($result['data'])): ?>
                    <tr><td colspan="10" class="text-center text-muted py-4">Aucune recette camion trouvÃ©e</td></tr>
                <?php endif; ?>
                </tbody>
            </table>
        </div></div>
...
<div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel"><div class="modal-dialog modal-lg"><div class="modal-content">
    <div class="modal-header"><h5 class="modal-title" id="addModalLabel"><i class="bi bi-truck me-2"></i>Nouvelle Recette Camion</h5><button type="button" class="btn-close" aria-label="Fermer" data-bs-dismiss="modal"></button></div>
        ...
        <div class="modal-body">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" inputmode="numeric" class="form-control amount-input" name="montant" id="create_montant" min="0" step="1" required placeholder="0">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-semibold">CatÃ©gorie <span class="text-danger">*</span></label>
                ...
                <div class="col-md-6">
                    <label class="form-label fw-semibold">Agence de dÃ©part <span class="text-danger">*</span></label>
                ...
                <div class="col-md-6">
                    <label class="form-label fw-semibold">Agence d'arrivÃ©e <span class="text-danger">*</span></label>
                ...
                <div class="col-md-6">
                    <label for="create_vehicule_id" class="form-label">Camion</label><select              <select name="vehicule_id" id="create_vehicule_id" class="form-select">
                        <option value="">-- SÃ©lectionner --</option>
                ...
                <div class="col-12">
                    <label for="create_description" class="form-label">Observation</label><textarea            <textarea name="description" id="create_description" class="form-control" rows="2" placeholder="Observation sur la recette..."></textarea>
                </div>
            </div>
        </div>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel"><div class="modal-dialog modal-lg"><div class="modal-content">
    <div class="modal-header"><h5 class="modal-title" id="editModalLabel"><i class="bi bi-pencil me-2"></i>Modifier Recette Camion</h5><button type="button" class="btn-close" aria-label="Fermer" data-bs-dismiss="modal"></button></div>
    <form method="POST" enctype="multipart/form-data" data-validate>
        ...
        <div class="modal-body">
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Montant (FCFA) <span class="text-danger">*</span></label>
                    <input type="text" inputmode="numeric" class="form-control amount-input" name="montant" id="edit_montant" min="0" step="1" required>
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-semibold">CatÃ©gorie <span class="text-danger">*</span></label>
                ...
                <div class="col-md-6">
                    <label for="edit_vehicule_id" class="form-label">Camion</label><select              <select name="vehicule_id" id="edit_vehicule_id" class="form-select">
                        <option value="">-- SÃ©lectionner --</option>
                        <?php foreach($vehicules as $v): ?>
                        <option value="<?php echo $v['id']; ?>"><?php echo e($v['immatriculation'] . ' - ' . $v['marque']); ?></option>
                        <?php endforeach; ?>
                ...
                <div class="col-12">
                    <label class="form-label">Observation</label><textarea            <textarea name="description" id="edit_description" class="form-control" rows="2"></textarea>
                </div>
            </div>
        </div>
        <div class="modal-footer">
...
// Category ID for "location_vehicule" â€” show client field only for this category
...
// AJAX submit for create form â€” keep modal open for continuous entry
document.getElementById('createForm')?.addEventListener('submit', function(e) {
    ...
    .then(function(data) {
        if (data.success) {
            ...
            alertDiv.innerHTML = '<i class="bi bi-exclamation-triangle me-2"></i>' + (data.message || 'Erreur lors de l\'enregistrement.') + '<button type="button" class="btn-close" aria-label="Fermer" data-bs-dismiss="alert"></button>';
            alertDiv.style.display = 'block';
            if (data.csrf_token) form.querySelector('[name="csrf_token"]').value = data.csrf_token;
        }
        btn.disabled = false;
