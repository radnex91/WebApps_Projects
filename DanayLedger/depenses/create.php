    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) { $error = 'Token invalide.'; }
        ...
        if (empty($date_depense) || $montant <= 0) { $error = 'La date et le montant sont obligatoires.'; }
            ...
            try {
                ...
                redirectWithMessage(APP_URL . '/depenses/', 'success', 'Dépense créée avec succès.');
...
<div class="main-content">
    ...
    <div class="page-content fade-in">
        ...
        <div class="page-header"><div><h1 class="page-title"><i class="bi bi-cart-dash me-2"></i>Nouvelle Dépense</h1></div><a href="index.php" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>Retour</a></div>
        <div class="card"><div class="card-body">
            <form method="POST" enctype="multipart/form-data" data-validate>
                <?php echo csrfField(); ?>
                <div class="row g-3">
                    <div class="col-md-6"><label class="form-label">Date <span class="text-danger">*</span></label><input type="date" name="date_depense" class="form-control" value="<?php echo e($_POST['date_depense'] ?? date('Y-m-d')); ?>" required></div>
                    <div class="col-md-6"><label class="form-label">Montant (FCFA) <span class="text-danger">*</span></label><input type="number" name="montant" class="form-control" min="0" step="1" value="<?php echo e($_POST['montant'] ?? ''); ?>" required></div>
                    <div class="col-md-6"><label class="form-label">Catégorie</label><select name="categorie_id" class="form-select"><option value="">-- Sélectionner --</option><?php foreach($categories as $c): ?><option value="<?php echo $c['id']; ?>" <?php echo (($_POST['categorie_id']??'')==$c['id']?'selected':''); ?>><?php echo e($c['nom']); ?></option><?php endforeach; ?></select></div>
                    <div class="col-md-6"><label class="form-label">Type d'opération</label><select name="type_operation_id" class="form-select"><option value="">-- Sélectionner --</option><?php foreach($typesOps as $t): ?><option value="<?php echo $t['id']; ?>" <?php echo (($_POST['type_operation_id']??'')==$t['id']?'selected':''); ?>><?php echo e($t['nom']); ?></option><?php endforeach; ?></select></div>
                    <div class="col-md-6"><label class="form-label">Agence</label><select name="agence_id" class="form-select"><option value="">-- Sélectionner --</option><?php foreach($agences as $a): ?><option value="<?php echo $a['id']; ?>" <?php echo (($_POST['agence_id']??'')==$a['id']?'selected':''); ?>><?php echo e($a['nomagence']); ?></option><?php endforeach; ?></select></div>
                    <div class="col-md-6"><label class="form-label">Camion</label><select name="vehicule_id" class="form-select"><option value="">-- Aucun --</option><?php foreach($vehicules as $v): ?><option value="<?php echo $v['id']; ?>" <?php echo (($_POST['vehicule_id']??'')==$v['id']?'selected':''); ?>><?php echo e($v['immatriculation'].' - '.$v['marque']); ?></option><?php endforeach; ?></select></div>
                    <div class="col-md-6"><label class="form-label">Nature</label><input type="text" name="nature" class="form-control" value="<?php echo e($_POST['nature'] ?? ''); ?>" placeholder="Ex: Carburant, Maintenance..."></div>
                    <div class="col-md-6"><label class="form-label">Justificatif</label><input type="file" name="justificatif" class="form-control" accept=".pdf,.jpg,.jpeg,.png"></div>
                    <div class="col-12"><label class="form-label">Description</label><textarea name="description" class="form-control" rows="3"><?php echo e($_POST['description'] ?? ''); ?></textarea></div>
