...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verifyCSRFToken($_POST['csrf_token'] ?? '')) {
    $id = (int)($_POST['id'] ?? 0);
    $action = $_POST['action'] ?? '';
    rateLimit('depenses_' . ($action ?: 'crud'));
    if ($action === 'validate' && hasPermission('depenses_validate')) {
        ...
        addAuditLog('cancel', 'depense', $id); setFlash('warning', 'DÃ©pense annulÃ©e.');
    } elseif ($action === 'delete' && hasPermission('depenses_delete')) {
        ...
        if ($oldRecord) {
            ...
            addAuditLog('delete', 'depense', $id, [], $oldRecord);
            setFlash('success', 'DÃ©pense supprimÃ©e.');
        } else {
            setFlash('error', 'DÃ©pense introuvable.');
        }
    }
    ...
    <div class="page-content fade-in">
        ...
        <div class="card mb-3"><div class="card-body">
            <form method="GET" class="row g-2 align-items-end">
                ...
                <div class="col-md-2"><label for="filter_categorie_id" class="form-label">Catégorie</label><select name="categorie_id" id="filter_categorie_id" class="form-select"><option value="">Toutes</option><?php foreach($categories as $c): ?><option value="<?php echo $c['id']; ?>" <?php echo $filterCategorie==$c['id']?'selected':''; ?>><?php echo e($c['nom']); ?></option><?php endforeach; ?></select></div>
        ...
        <div class="card"><div class="table-container">
            <table class="table" aria-label="Liste des dépenses">
                ...
                <tr>
                    ...
                    <td><div class="action-btns">
                        <a href="view.php?id=<?php echo $d['id']; ?>" class="btn btn-sm btn-outline-primary" aria-label="Voir"><i class="bi bi-eye"></i></a>
                        <?php if ($d['statut']==='en_attente' && hasPermission('depenses_edit')): ?>
                        <a href="edit.php?id=<?php echo $d['id']; ?>" class="btn btn-sm btn-outline-warning" aria-label="Modifier"><i class="bi bi-pencil"></i></a>
                        <?php endif; ?>
                        <?php if ($d['statut']==='en_attente' && hasPermission('depenses_validate')): ?>
                        <form method="POST" style="display:inline"><input type="hidden" name="csrf_token" value="<?php echo generateCSRFToken(); ?>"><input type="hidden" name="id" value="<?php echo $d['id']; ?>"><input type="hidden" name="action" value="validate"><button type="submit" class="btn btn-sm btn-outline-success" aria-label="Valider" data-confirm="Valider ?"><i class="bi bi-check-lg"></i></button></form>
                        <form method="POST" style="display:inline"><input type="hidden" name="csrf_token" value="<?php echo generateCSRFToken(); ?>"><input type="hidden" name="id" value="<?php echo $d['id']; ?>"><input type="hidden" name="action" value="cancel"><button type="submit" class="btn btn-sm btn-outline-danger" aria-label="Supprimer" data-confirm="Annuler ?"><i class="bi bi-x-lg"></i></button></form>
                        <?php endif; ?>
                        <?php if (hasPermission('depenses_delete')): ?>
                        <form method="POST" style="display:inline"><input type="hidden" name="csrf_token" value="<?php echo generateCSRFToken(); ?>"><input type="hidden" name="id" value="<?php echo $d['id']; ?>"><input type="hidden" name="action" value="delete"><button type="submit" class="btn btn-sm btn-outline-danger" aria-label="Supprimer" data-confirm="Supprimer cette dÃ©pense ?" title="Supprimer"><i class="bi bi-trash"></i></button></form>
                        <?php endif; ?>
                    </div></td>
                </tr>
                <?php endforeach; ?>
