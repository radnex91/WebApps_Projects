<?php
$pageTitle = 'Recherche Multicritère';
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && !empty($_GET)) {
    ...
    $type = $_GET['type'] ?? '';
    $montantMin = (float)($_GET['montant_min'] ?? 0) ?: null;
    $montantMax = (float)($_GET['montant_max'] ?? 0) ?: null;
...
$sourceLabels = ['recette' => 'Recette', 'depense' => 'Dépense', 'recette_camion' => 'Recette camion', 'versement' => 'Versement'];
...
<div class="main-content">
    ...
    <div class="page-content fade-in">
        <div class="page-header"><div><h1 class="page-title"><i class="bi bi-search me-2"></i>Recherche Multicritère</h1><p class="page-subtitle">Recherchez dans toutes les opérations financières</p></div></div>

        <div class="card mb-3"><div class="card-body">
            <form method="GET" class="row g-2 align-items-end">
                <div class="col-md-2"><label class="form-label">Du</label><input type="date" name="date_from" value="<?php echo e($_GET['date_from'] ?? ''); ?>" class="form-control"></div>
                <div class="col-md-2"><label class="form-label">Au</label><input type="date" name="date_to" value="<?php echo e($_GET['date_to'] ?? ''); ?>" class="form-control"></div>
                <div class="col-md-2"><label class="form-label">Agence</label><select name="agence_id" class="form-select"><option value="">Toutes</option><?php foreach($agences as $a): ?><option value="<?php echo $a['id']; ?>" <?php echo (($_GET['agence_id']??'')==$a['id']?'selected':''); ?>><?php echo e($a['nomagence']); ?></option><?php endforeach; ?></select></div>
                <div class="col-md-2"><label class="form-label">Type</label><select name="search_type" class="form-select"><option value="all">Tous</option><option value="recettes" <?php echo (($_GET['search_type']??'')==='recettes'?'selected':''); ?>>Recettes</option><option value="depenses" <?php echo (($_GET['search_type']??'')==='depenses'?'selected':''); ?>>Dépenses</option><option value="recettes_camions" <?php echo (($_GET['search_type']??'')==='recettes_camions'?'selected':''); ?>>Recettes camions</option><option value="versements" <?php echo (($_GET['search_type']??'')==='versements'?'selected':''); ?>>Versements</option></select></div>
                <div class="col-md-2"><label class="form-label">Montant min</label><input type="number" name="montant_min" value="<?php echo e($_GET['montant_min'] ?? ''); ?>" class="form-control" min="0"></div>
                <div class="col-md-2"><label class="form-label">Montant max</label><input type="number" name="montant_max" value="<?php echo e($_GET['montant_max'] ?? ''); ?>" class="form-control" min="0"></div>
                <div class="col-md-2"><label class="form-label">Camion</label><select name="vehicule_id" class="form-select"><option value="">Tous</option><?php foreach($vehicules as $v): ?><option value="<?php echo $v['id']; ?>" <?php echo (($_GET['vehicule_id']??'')==$v['id']?'selected':''); ?>><?php echo e($v['immatriculation']); ?></option><?php endforeach; ?></select></div>
                <div class="col-md-2"><label class="form-label">Statut</label><select name="statut" class="form-select"><option value="">Tous</option><?php foreach(STATUSES as $k=>$l): ?><option value="<?php echo $k; ?>" <?php echo (($_GET['statut']??'')===$k?'selected':''); ?>><?php echo e($l); ?></option><?php endforeach; ?></select></div>
                <div class="col-md-3"><label class="form-label">Mot-clé</label><input type="text" name="keyword" value="<?php echo e($_GET['keyword'] ?? ''); ?>" class="form-control" placeholder="Référence, description..."></div>
                <div class="col-md-2"><button type="submit" class="btn btn-primary w-100"><i class="bi bi-search me-1"></i>Rechercher</button></div>
                <div class="col-md-1"><a href="index.php" class="btn btn-outline-secondary w-100" title="Réinitialiser"><i class="bi bi-arrow-counterclockwise"></i></a></div>
            </form>
        </div></div>

        <?php if ($searchPerformed): ?>
        <div class="d-flex justify-content-between align-items-center mb-2">
            <span class="text-muted"><?php echo count($results); ?> résultat(s) trouvé(s)</span>
        ...
        <div class="card"><div class="table-container">
            <table class="table" id="searchTable">
                <thead><tr><th>Type</th><th>Référence</th><th>Date</th><th>Montant</th><th>Agence</th><th>Catégorie</th><th>Camion</th><th>Statut</th><th>Actions</th></tr></thead>
                <tbody>
                ...
                <tr>
                    ...
                    <td><small><?php
                        if ($r['source'] === 'recette_camion') {
                            ...
                            echo e($dep && $arr ? "$dep → $arr" : ($dep ?: ($arr ?: '-')));
                        } else {
                            echo e($r['agence_nom'] ?? '-');
                        }
                    ?></small></td>
                ...
                <?php endforeach; ?>
                <?php if (empty($results)): ?><tr><td colspan="9" class="text-center text-muted py-4">Aucun résultat</td></tr><?php endif; ?>
                </tbody>
            </table>
        </div></div>
        <?php endif; ?>
