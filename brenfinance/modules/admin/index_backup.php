...
if (!canSeeModule('admin')) { flash('danger','Vous n\'avez pas accès à ce module.'); header('Location: '.BASE_URL.'/dashboard.php'); exit; }
...
$permModules = [
  ...
  'tresorerie'        => ['label' => 'Trésorerie',         'actions' => ['saisir' => 'Saisir opérations', 'consulter' => 'Consulter', 'rapprocher' => 'Rapprochement']],
          ...
          <tbody>
            ...
            <tr data-id="<?= $u['id'] ?>" data-role="<?= $u['role_id'] ?>" data-caisse="<?= $u['caisse_id']??'' ?>">
              ...
              <td style="white-space:nowrap">
                <button class="btn btn-ghost btn-sm" onclick="editUser(<?= $u['id'] ?>, '<?= sanitize(addslashes($u['nom'])) ?>', '<?= sanitize(addslashes($u['prenom'])) ?>', '<?= sanitize(addslashes($u['matricule']??'')) ?>', '<?= sanitize(addslashes($u['email'])) ?>', '<?= sanitize(addslashes($u['telephone']??'')) ?>', <?= $u['agence_id']??'null' ?>, <?= $u['service_id']??'null' ?>, <?= $u['service_responsable_id']??'null' ?>)">✎</button>
    ...
    <div class="card" style="max-width:600px">
      ...
      <form method="post" enctype="multipart/form-data">
        ...
        <div class="card-body">
          ...
          <div class="form-group">
            <label class="form-label">🎨 Moteur de Thème Ultra-Moderne</label>
            <div style="background:var(--surface2);border-radius:var(--radius-lg);padding:20px;border:1px solid var(--border)">
              <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:12px;margin-bottom:16px" id="theme-grid">
                <?php
                $themes = [
                  'default'    => ['BrenFinance',    '#1a4f8a','#0ea87e', '🏦'],
                  'wallstreet' => ['Wall Street',    '#0a2540','#00ff88', '💹'],
                  'cyberpunk'  => ['Cyberpunk',      '#6c00ff','#00f0ff', '🔮'],
                  'aurora'     => ['Aurora',         '#065535','#22d4ee', '🌌'],
                  'executive'  => ['Executive',      '#001d3d','#ffd60a', '💼'],
                  'solar'      => ['Solar Flare',    '#c2410c','#fbbf24', '☀️'],
                  'ocean'      => ['Ocean Depth',    '#003566','#48cae4', '🌊'],
                  'royal'      => ['Royal Purple',   '#4a0072','#ffd700', '👑'],
                  'emerald'    => ['Emerald Luxe',   '#064e3b','#10b981', '💎'],
                ];
                ...
                <div class="theme-swatch" data-theme="<?= $val ?>"
                  ...
                  <?php if ($curTheme===$val): ?>
                  <div style="position:absolute;top:8px;right:8px;background:var(--accent);color:#fff;border-radius:50%;width:20px;height:20px;display:flex;align-items:center;justify-content:center;font-size:12px">✓</div>
                  <?php endif; ?>
                </div>
                <?php endforeach; ?>
              </div>
              ...
              <div style="font-size:12px;color:var(--text3);text-align:center">
                💡 Cliquez sur un thème pour la prévisualisation en direct • Le choix est sauvegardé automatiquement
              </div>
            </div>
          </div>
  ...
  <div class="tab-content <?= $activeTab==='roles'?'active':'' ?>" id="tab-roles">
    ...
    <div class="card mb-24">
      <div class="card-header">
        ...
        <a href="index.php?tab=roles" class="btn btn-ghost btn-sm" style="margin-left:auto">← Retour</a>
      </div>
      <form method="post">
